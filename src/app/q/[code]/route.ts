...
export async function GET(
  ...
  try {
    ...
    if (!city || !country) {
      ...
      // If still null (e.g. rate limit, local IP, or network error), leave as null
      if (!city || !country) {
        city = city || null;
    ...
    // Set short-lived cookies (1 day) to track scan metadata on the client
    response.cookies.set("last_scan_device", deviceType, { path: "/", maxAge: 86400 });
    response.cookies.set("last_scan_city", city || "", { path: "/", maxAge: 86400 });
    response.cookies.set("last_scan_country", country || "", { path: "/", maxAge: 86400 });
    response.cookies.set("last_scan_ip", ip, { path: "/", maxAge: 86400 });
    response.cookies.set("last_scan_qr", qrCode.qrCodeId, { path: "/", maxAge: 86400 });
